...
runs each directory via `pkg/runner.TestDir`. There are no per-analyzer
...
| Path | What it is |
|------|------------|
| `pkg/dsl/`               | Go structs mirroring the CUE schema. `dsl.Arg` is a sum of `string` and `[]string` for predicate args. |
| `pkg/loader/`            | CUE loader. Embeds the built-in `github.com/imjasonh/pasta` module under `pkg/loader/cuemod/`. |
| `pkg/loader/cuemod/`     | The embedded built-in CUE module: `schema/`, `lang/<name>/`, `patterns/<name>/`. |
| `pkg/lang/`              | Runtime language registry. `grammars.go` is the only Go-side language code (maps grammar name → tree-sitter `GetLanguage`). |
| `pkg/tsutil/`            | gotreesitter `Node` wrapper that carries source bytes + language, so callers don't have to thread them. |
| `pkg/match/`             | Pattern matcher: node unions, fields, adjacent windows, preceding, predicates (positional), checks (named). |
| `pkg/factstore/`         | Per-run fact store with dual indexing — by (kind, byte-range) and by (kind, identifier-text). |
| `pkg/effect/`            | Compiles edits to byte-range ops, handles `@capture` interpolation, comment preservation, and `trim_start`/`trim_end`. |
| `pkg/apply/`             | Applies ops to source bytes with conflict detection. |
| `pkg/engine/`            | Top-level orchestrator. SCC scheduler with fixpoint groups for cyclic rule deps. |
| `pkg/runner/`            | Programmatic API used by both the CLI and Go tests. `LoadRules`, `RunFile`, `TestDir`. |
...
1. **The grammar is already linked in.** Add a CUE file under
   `pkg/loader/cuemod/lang/<name>/<name>.cue` declaring the
...
2. **The grammar isn't linked in yet.** Add an entry to
   `pkg/lang/grammars.go` mapping the new grammar name to its
...
- **Rules are pure CUE.** No per-analyzer Go code. All semantic checks
  go through the predicate / check registry in `pkg/match/predicate.go`.
...
- **Schema-first.** When adding a predicate or extending an edit form,
  update both `pkg/loader/cuemod/schema/schema.cue` AND the
